...
sed -i "
     ...
     94s/.*/start_rtl_command=\"start_rtl_command=\"play_sdr -b 600 -s {samp_rate} -f {center_freq} -x 16 -g {rf_gain} -y 0 -\".format(rf_gain=rf_gain, center_freq=center_freq, samp_rate=samp_rate, ppm=ppm)/;
     137s/.*/client_audio_buffer_size = 10/;
     143s/.*/start_mod = \"$mode\"/;
     150s/.*/waterfall_min_level = -100/;
...
OUTPUT=$(zenity --forms --title="OpenWebRX - SDRPlay" --text="Enter the SDR start-up parameters." \
--separator="," --add-entry="Frequency (Hz)" --add-entry="Mode (am,fm,usb,lsb,cw)" --add-entry="Freq Correction (ppm)")
...
start
killall -9 openwebrx rtl_mus rtl_sdr csdr play_sdr
