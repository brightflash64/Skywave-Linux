#!/bin/bash
#OpenWebRX for Direct Sampling RTL-SDR Devices
start() {
...
sed -i "
     ...
     94s/.*/start_rtl_command=\"rtl_sdr -D {adc} -s {samp_rate} -f {center_freq} -p {ppm} -g {rf_gain} -\".format(rf_gain=rf_gain, center_freq=center_freq, adc=adc, samp_rate=samp_rate, ppm=ppm)/;
...
OUTPUT=$(zenity --forms --title="OpenWebRX - RTL-SDR Direct Sampling" --text="Enter the SDR start-up parameters." \
--separator="," --add-entry="Center Frequency (MHz)" --add-entry="Mode (am,fm,usb,lsb,cw)" --add-entry="Freq Correction (ppm)" \
--add-entry="I or Q ADC? (1 = I, 2 = Q)")
